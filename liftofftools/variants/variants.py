...
def analyze_variants(ref_proteins, target_proteins, ref_trans, target_trans, target_db):
                ...
                if target_db.db_connection[tran].featuretype == "gene":
                    ...
                    if  source_name in ref_trans:
                        ...
                        if tran in target_proteins and source_name in ref_proteins:
                            ...
                            variant_effect = find_variants(align_dna, seq_id_dna, seq_id_protein)
                        else:
                            variant_effect, seq_id_protein = 'NA' , 'NA'
                        write_output(source_name, tran, seq_id_dna, seq_id_protein, variant_effect, of_ve)
            add_unmapped_trans(ref_trans, target_trans, ref_proteins, of_ve)
...
def find_variants(align_dna, seq_id_dna, seq_id_protein):
    if seq_id_dna == 1.0:
        return 'identical'
    ...
    if is_frameshift(target_align):
        return 'frameshift'
    if has_stop_codon(ref_align, target_align):
        return 'stop_gained'
    if "-" in ref_align:
...
def add_unmapped_trans(ref_trans, target_trans, ref_proteins, output_file):
    for tran in ref_trans:
        if tran not in target_trans:
